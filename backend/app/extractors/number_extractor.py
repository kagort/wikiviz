"""
 Извлекает числовые данные (§17 ТЗ) ИСКЛЮЧИТЕЛЬНО из infobox
    ...
    парами "число%+описание" обрабатываются отдельно.
        ...
        Известные ограничения (сознательно не устраняются дальнейшим
    усложнением regex - экстраполируя опыт этой фазы, каждое такое
    исправление открывает следующий edge case; дальнейшая очистка -
    задача будущего NLP/Transformation слоя, см. §1 ТЗ, а не этого
    extractor'а):
    - группировка полагается на <th class="infobox-header">; там, где
      разметка не следует этому паттерну (обнаружено на статье о
      Франции на русском - вся секция помечена одной случайной группой
      вместо содержательных подзаголовков), label остаётся не вполне
      осмысленным, но НЕ ломает структуру данных;
    - коды (телефонный, ISO, валютный), встроенные в текст без даты и
      без координат, могут извлекаться как формально валидные, но
      семантически бессмысленные значения;
    - unit иногда захватывает случайное следующее слово из "рыхлой"
      прозы ячейки, а не единицу измерения в строгом смысле;
    - строка без собственного заголовка группы, идущая сразу после
      чужой смысловой секции, наследует её label по ошибке.

    Все перечисленные случаи дают семантический шум (неверная
    группировка/unit), а не структурную поломку (crash, пропавшее
    значение) - NumericValue остаётся валидным объектом контракта в
    любом случае. Их устранение перенесено на будущий NLP/Transformation
    слой (§1 ТЗ), который будет работать поверх уже извлечённых
    NumericValue, а не встраиваться в сам regex-based extractor.
    """
